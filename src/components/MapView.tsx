...
const MapView = ({ layers, onFeatureClick }: MapViewProps) => {
  ...
  useEffect(() => {
    ...
    if (vulnerableLayer?.enabled) {
      loadIBGEMunicipalities();
    } else {
      removeIBGELayer();
    }
  }, [layers, mapLoaded]);

  const loadIBGEMunicipalities = async () => {
    if (!map.current) return;

    try {
      toast.info("Carregando municípios...", {
        description: "Buscando dados do IBGE"
      });
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/get-ibge-municipalities`,
        {
          ...
          body: JSON.stringify({ state: 'PA' }) // Pará
        }
      );
      ...
      const sourceId = 'ibge-municipalities';
      const layerId = 'ibge-municipalities-fill';
      const outlineId = 'ibge-municipalities-outline';
      ...
      mapInstance.addLayer({
        ...
        paint: {
          ...
          'fill-opacity': (layers.find(l => l.id === 'vulnerable-communities')?.opacity || 65) / 100
        }
      });
      ...
      mapInstance.on('click', layerId, (e) => {
        if (e.features && e.features[0]) {
          ...
          onFeatureClick({
            name: props.nome || props.name || 'Município',
      ...
      toast.success("Municípios carregados", {
        description: `${result.count} municípios visualizados`
      });
  ...
  const removeIBGELayer = () => {
    ...
    try {
      ...
      if (mapInstance.getLayer(outlineId)) mapInstance.removeLayer(outlineId);
      if (mapInstance.getSource(sourceId)) mapInstance.removeSource(sourceId);
      console.log("IBGE layer removed");
    } catch (error) {
      console.error("Error removing IBGE layer:", error);
    }
  };
